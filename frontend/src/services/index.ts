...
export const apiDetectCamera = (imageBase64?: string) =>
  req<{ detection: MoodEntry; entryId: string; dailyLimit: DailyLimitInfo }>('/detect/camera', {
...
export const apiDetectUpload = (file: File) => {
  ...
  return fetch(`${BASE}/detect/upload`, {
    ...
    detection: MoodEntry; entryId: string; imageUrl: string; dailyLimit: DailyLimitInfo;
  }>>;
};
...
export const apiBulkDeleteHistory = (mode: 'all'|'today'|'week'|'month'|'ids', ids?: string[]) =>
  req<{ deletedCount: number }>('/history/bulk', {
    method: 'DELETE', body: JSON.stringify({ mode, ids }),
  });

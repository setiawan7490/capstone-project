...
import { getMoodHistory, getMoodEntryById, deleteMoodEntry, bulkDeleteMoodEntries } from '../services/historyService';
...
export const getHistory = async (req: Request, res: Response): Promise<void> => {
  ...
  } catch (e) { res.status(500).json({ success:false, message:'Failed', error:(e as Error).message }); }
};
...
export const bulkDeleteHistory = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ success:true, message:`${count} riwayat berhasil dihapus`, data:{ deletedCount:count } });
  } catch (e) { res.status(500).json({ success:false, message:'Bulk delete failed', error:(e as Error).message }); }
};